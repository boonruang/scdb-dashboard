...
import { getManufacturerByKeyword } from '../actions/manufacturermap.action'
      ...
      <Box  backgroundColor={colors.primary[400]} key={result.properties.Id} sx={{ m: 1 }} >
          ...
          >
            ...
            <Box display="flex" flexDirection="column" justifyContent="center" sx={{ ml: 2 }} >
              <Box>{result.properties.organization} {result.properties.establishmentno}</Box>
...
const ManufacturerList = ({searchTerm}) => {
  const theme = useTheme()
  const colors = tokens(theme.palette.mode)
  ...
  useEffect(() => {
    dispatch(getManufacturerByKeyword(searchTerm))
    console.log('useEffect is called', searchTerm)
  },[dispatch,searchTerm])

  const { result, isFetching, isError } = useSelector((state) => state.app.manufacturermapReducer)
  if (result) {
