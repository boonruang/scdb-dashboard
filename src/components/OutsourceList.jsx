...
import { getOutsourceByKeyword } from '../actions/outsourcemap.action'
      ...
      <Box  backgroundColor={colors.primary[400]} key={result.properties.Id} sx={{ m: 1 }} >
          ...
          >
            ...
            <Box display="flex" flexDirection="column" justifyContent="center" sx={{ ml: 2 }} >
              <Box>{result.properties.services} {result.properties.remark} {result.properties.standard}</Box>
...
const OutsourceList = ({searchTerm}) => {
  const theme = useTheme()
  const colors = tokens(theme.palette.mode)
  ...
  useEffect(() => {
    dispatch(getOutsourceByKeyword(searchTerm))
    console.log('useEffect is called', searchTerm)
  },[dispatch,searchTerm])

  const { result, isFetching, isError } = useSelector((state) => state.app.outsourcemapReducer)
  if (result) {
