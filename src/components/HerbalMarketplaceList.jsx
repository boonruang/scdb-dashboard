...
import { getHerbalmarketplaceByKeyword } from '../actions/herbalmarketplacemap.action'
      ...
      <Box  backgroundColor={colors.primary[400]} key={result.properties.Id} sx={{ m: 1 }} >
          ...
          >
            ...
            <Box display="flex" flexDirection="column" justifyContent="center" sx={{ ml: 2 }} >
              <Box>{result.properties.herbals} {result.properties.tambon}</Box>
...
const HerbalMarketplaceList = ({searchTerm}) => {
  const theme = useTheme()
  const colors = tokens(theme.palette.mode)
  ...
  useEffect(() => {
    dispatch(getHerbalmarketplaceByKeyword(searchTerm))
    console.log('useEffect is called', searchTerm)
  },[dispatch,searchTerm])

  const { result, isFetching, isError } = useSelector((state) => state.app.herbalmarketplacemapReducer)
  if (result) {
...
export default HerbalMarketplaceList