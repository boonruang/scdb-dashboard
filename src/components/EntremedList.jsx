...
import { getEntrepreneurmedmapByKeyword } from '../actions/entrepreneumedicalmap.action'
      ...
      <Box  backgroundColor={colors.primary[400]} key={result.properties.Id} sx={{ m: 1 }} >
          ...
          >
            ...
            <Box display="flex" flexDirection="column" justifyContent="center" sx={{ ml: 2 }} >
              <Box>{result.properties.service_type} {result.properties.owner} {result.properties.reference}</Box>
              <Box>{result.properties.tambon} {result.properties.amphoe} {result.properties.province}</Box>
            </Box> 
          </Box>             
      </Box>
...
const EntremedList = ({searchTerm}) => {
  ...
  useEffect(() => {
    dispatch(getEntrepreneurmedmapByKeyword(searchTerm))
    console.log('useEffect is called', searchTerm)
  },[dispatch,searchTerm])

  const { result, isFetching, isError } = useSelector((state) => state.app.entrepreneumedicalmapReducer)
  if (result) {
