...
import { getDocument, deleteDocument } from '../../actions/document.action'
...
const Documents = () => {
    ...
    const { result, isFetching } = useSelector((state) => state.app.documentReducer)
    ...
    const handleDeleteClick = ({state}) => {
        setRowId(state.row.document_id)
        setOpen(true)
    }
    ...
    {
      field: 'date_received',
      headerName: 'วันที่ได้รับเอกสาร',
      flex: 0.8,
      cellClassName: "name-column--cell"
    },
    {
      field: 'doc_from',
      headerName: 'เอกสารจาก',
      flex: 0.8,
      cellClassName: "name-column--cell"
    },
    {
      field: 'doc_to',
      headerName: 'เอกสารถึง',
      flex: 0.8,
      cellClassName: "name-column--cell"
    },
    {
    ...
    },    
    {
      ...
      flex: 0.8,
      renderCell: (params) => {
        return params.row.Project?.project_name || 'N/A';
      }
    },     
    { field: 'actions', headerName: 'ดำเนินการ', headerAlign: 'center', align: 'center', flex: 1.5, renderCell: (params) => {
        return (
          <Box>
            <Button
              onClick={() => (navigate('/document/detail',  { state: { row: params.row }} ))}
              variant="outlined"
              color="success"
            >
              รายละเอียด
        ...
        { loginReducer?.result?.roles?.find((role) => [ROLES.Admin,ROLES.Editor].includes(role))
            ? <Button
            onClick={() => (navigate('/document/edit',  { state: { row: params.row }} ))}
              variant="outlined"
              color="info"
              sx={{ ml: 1 }}            
            >
