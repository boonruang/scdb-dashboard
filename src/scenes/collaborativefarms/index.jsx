...
const Collaborativefarms = () => {
    ...
    const [collaborativefarmId, setCollaborativefarmId] = useState([])
    ...
    const handleAddButton = () => {
        navigate('/collaborativefarm/add')
    };
    ...
    const handleDeleteClick = ({state}) => {
        ...
        setCollaborativefarmId(state.row.id)
        setOpen(true)
    ...
    // XLSX.writeFile(wb, "collaborativefarm.xlsx")
    ...
    const columns = [
        ...
        { field: 'actions', headerName: 'ดำเนินการ', headerAlign: 'center', align: 'center', flex: 1.5, renderCell: (params) => {
            return (
              <Box>
                <Button
                  onClick={() => (navigate('/collaborativefarm/detail',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="success"
                >
                  รายละเอียด
            ...
            { loginReducer?.result?.roles?.find((role) => [ROLES.Admin,ROLES.Editor].includes(role))
                ? <Button
                onClick={() => (navigate('/collaborativefarm/edit',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="info"
                  sx={{ ml: 1 }}            
                >
        ...
        <Box m="20px">
            ...
            <Box m="40px 0 0 0" height="75vh" sx={{
                ...
                <Box display="flex" justifyContent="end">
                    ...
                    <Box display="flex" justifyContent="end">
                        <ExportExcelButton
                        action={() => getCollaborativefarms({ all: true })}
                        selector={(state) => state.app.collaborativefarmReducer.result}
                        fileName="collaborativefarms.xlsx"
                        mapRow={(item) => ({
                            ...item,
                            herbalNames: Array.isArray(item.herbals) && item.herbals.length > 0
                            ? item.herbals.map(h => h.herbalname).join(', ')
