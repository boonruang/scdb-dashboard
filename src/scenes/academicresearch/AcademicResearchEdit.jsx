...
import { updateAcademicResearch } from '../../actions/academicResearch.action'
import { useNavigate, useLocation } from 'react-router-dom'
import MessageBox from 'components/MessageBox'

var DEGREE_OPTIONS = ['ปริญญาโท', 'ปริญญาเอก']

var AcademicResearchEdit = function(props) {
  ...
  return (
    <Box m="20px">
      <Header title="แก้ไขข้อมูลวิจัย" />
      <Formik
        initialValues={initialValues}
        validationSchema={schema}
        onSubmit={async function(values, helpers) {
          if (submitted) return
          var res = await dispatch(updateAcademicResearch(row.research_id, values))
          if (res && res.success) {
...
export default AcademicResearchEdit
