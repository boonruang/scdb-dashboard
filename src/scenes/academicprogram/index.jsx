...
import { getAcademicProgram, deleteAcademicProgram } from '../../actions/academicProgram.action'
...
const AcademicProgram = () => {
    ...
    const { result, isFetching } = useSelector((state) => state.app.academicProgramReducer)

    const loginReducer = useSelector((state) => state.app.loginReducer)   
    
    const DeleteFunction = () => {
        dispatch(deleteAcademicProgram(rowId))
        setOpen(false)
    }
    ...
    const columns = [
        ...
        { field: 'actions', headerName: 'ดำเนินการ', headerAlign: 'center', align: 'center', flex: 1, renderCell: (params) => {
            return (
              <Box>
                <Button
                  onClick={() => (navigate('/academicprogram/detail',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="success"
                >
                  รายละเอียด
            ...
            { loginReducer?.result?.roles?.find((role) => [ROLES.Admin,ROLES.Editor].includes(role))
                ? <Button
                onClick={() => (navigate('/academicprogram/edit',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="info"
                  sx={{ ml: 1 }}            
                >
