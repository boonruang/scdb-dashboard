...
import { getOutsources, deleteOutsource } from '../../actions/outsource.action'
...
const Outsources = () => {
    ...
    useEffect(() => {
        dispatch(getOutsources())
    },[dispatch])


    const { result, isFetching } = useSelector((state) => state.app.outsourceReducer)

    const loginReducer = useSelector((state) => state.app.loginReducer)   
    

    const DeleteFunction = () => {
        dispatch(deleteOutsource(rowId))
        setOpen(false)
    }
    ...
    const columns = [
        ...
        {
            field: 'service',
            headerName: 'โครงสร้างหรือสาธารณูปโภค',
            flex: 0.6,
            cellClassName: "name-column--cell",
            renderCell: (params) => {
                // console.log('params',params.row.status);
                if (params.row.services === null || params.row.services === '') {
                    return (<Box>ไม่ระบุ</Box>);
                }  else {
                    return (<Box >{params.row.services}</Box>);
                }
                },
        }, 
        { field: 'actions', headerName: 'ดำเนินการ', headerAlign: 'center', align: 'center', flex: 1, renderCell: (params) => {
            return (
              <Box>
                ...
                //   onClick={() => (navigate('/farmers/detail',  { state: { id: params.row.id }} ))}
                  onClick={() => (navigate('/outsources/detail',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="success"
                >
                  รายละเอียด
            ...
            { loginReducer?.result?.roles?.find((role) => [ROLES.Admin,ROLES.Editor].includes(role))
                ? <Button
                onClick={() => (navigate('/outsources/edit',  { state: { row: params.row }} ))}
                  variant="outlined"
                  color="info"
                  sx={{ ml: 1 }}            
                >
    ...
    return (
        <Box m="20px">
            <Header title="แหล่งแปรรูป" subtitle="รายการข้อมูลแหล่งแปรรูปขั้นต้น" />
            <Box m="40px 0 0 0" height="75vh" sx={{
                ...
                "& .MuiDataGrid-toolbarContainer .MuiButton-text": {
                    ...
                    <Box display="flex" justifyContent="end">
                        { loginReducer?.result?.roles?.find((role) => [ROLES.Admin,ROLES.Editor].includes(role)) 
                        ? <Box display="flex" justifyContent="end" >
                            <Button  onClick={() => navigate('/outsources/add')}  
                                sx={{
